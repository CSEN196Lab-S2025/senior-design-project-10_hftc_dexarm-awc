...
class Dexarm:
    ...
    def _send_cmd(self, data, wait=True):
        ...
        while True:
            serial_str = self.ser.readline().decode('utf-8') #s() #.decode("utf-8")
            # for x in serial_str:
            #     x = x.decode("utf-8")
            if len(serial_str) > 0:   
    ...
    """Sliding Rail"""
    def sliding_rail_init(self, Z):
        """
        Sliding rail init.
        """
        #print(Z)
        Z = 50
        self._send_cmd(f"M914 X30 Y10 Z70 E70\r")
        self._send_cmd(f"M2005 X30 Y10 Z{Z} E{Z}\r")

    """ Rotary Module"""
    def rotate_init(self):
        # """ the first calls the rotary module and rotates it"""
        # self._send_cmd("M888 P6\r") 
        # """ the second command initializes the dexarm"""
        # self._send_cmd("M2100\r")
        self.rotate_to_position(0)
    ...
    def rotate_to_position(self, position):
        ...
        # the_angle = int(decoded[18:])
        # print('angle? : ', the_angle)

        # while the_angle != position: 
        #     maybe_angle = self._send_cmd(f"M2101\r")
        #     decoded = maybe_angle[0].decode('utf-8').strip()
        #     print('decoded: ', decoded)
        #     the_angle = int(decoded[18:])
        #     print('angle? : ', the_angle)
        ...
        #self._send_cmd(f"M2101\r")
        # while True:
        #     angle = self._send_cmd("M2101\r")
        #     print("fuck")
        #     #print("angle: "+angle)
        
        #     if angle == None and len(re.findall(r'\d+', angle)) == 0 and angle is not "ok":
        #         #continue
        #         print("angle: "+angle)
        #         print("Position: " + re.findall(r'\d+', angle)[0] + ", CMD_POS: " + str(position))
        #         #break
        #         if angle == position:
        #             break
        #     else:
        #         continue
        #     print(re.findall(r'\d+', angle))
        #     if position-1%360 <= int(re.findall(r'\d+', angle)) <= position+1:
        #         break
            #time.sleep(0.1)
